...
public class RoomPanel extends JPanel implements MouseListener {
    ...
    private JLabel weeksDisplay, infectDisplay, infectionRatioDisplay, avgImmuneDisplay, statusDisplay, deathsDisplay;
    ...
    //row and column of the current block (the one the mouse hovers over); if mouse is not over any block, currRow==currCol==-1
    private int currRow, currCol; 
    private int runState = 0;  //if runState==0, auto-run is off; if runState==1, auto-run is on
    //A swing.Timer to run the program automatically with the StepListener, first argument is delay time in milliseconds
    private Timer timer = new Timer(1, new StepListener());

    public RoomPanel() {
        setLayout(new BorderLayout());
        
        // initialized a grid board, or "room", 
        // of size 50 by 50 (or any other specified size, code is made to be versatile); each block is a person
        initialize(200,200);
        ...
        avgImmuneDisplay = new JLabel("Average Immunity: " + 
            ((int)(100*(1.0*Person.totalImmunity/(numRows*numCols)))/100.0) + 
            "     ", SwingConstants.CENTER);  //Spread becomes limited and eventually stops as average immunity increases
        deathsDisplay = new JLabel("Cumulative Deaths: " + Person.deaths);
        NORTH.add(weeksDisplay);
        NORTH.add(infectDisplay);
        NORTH.add(infectionRatioDisplay);
        NORTH.add(avgImmuneDisplay);
        NORTH.add(deathsDisplay);
     ...
     * Method is not absolutely necessary, but I think it makes the initialization process more concise
     */
    ...
    //updates the color of the entire grid for each step of a week; basically, updates color
        ...
        for(int r = 0; r < numRows; r++) {
            for(int c = 0; c < numCols; c++) {
                if(people[r][c].isInfected()) {
                    // infection stage sets color of person
                    Color redd = new Color((int) (255 / (1 + people[r][c].getSicknessStage() / 2)), 0, 0);
                    grid[r][c].setBackground(redd);
                }else {
                    Color bblue = new Color(0, 0, (int) (255 * people[r][c].getImmuneRate() / 90));
                    grid[r][c].setBackground(bblue);
                }
            }
        }
    }
    ...
    //Minor methods BEGIN
    //simple way to add a button with a specific text and ActionListener
    ...
    //simple way to update status board, which is a pain to format
    private void updateStatusDisplay(int r, int c, Person p, boolean reset) {
        //if reset is true, disregard the three prior arguments and reset statusDisplay
        if(reset) {
            ...
            statusDisplay.setText(
                "<html>Row: " + r + "<br>Col: " + c + 
                "<br>" + p.toString() + 
                "<br>Immune Rate: " + p.getImmuneRate() + 
                "<br>Infect Rate: " + p.getInfectRate() + 
                "<br>Stage: " + p.getSicknessStage() + "</html>");
        }
    }
    ...
    private void updateTopDisplay() {
        ...
        deathsDisplay.setText("Cumulative Deaths: " + Person.deaths);
    }

    /**
     * Count number of infected/red tiles
     * @return the number of currently infected persons
     */
    private int countNumInfected() {
        // WRITE THIS METHOD
        int count = 0;
    ...
    // Resets entire grid[][] to blue, all Persons of people[][] to healthy
    private void reset() {
        /**
         * WRITE THIS METHOD
         *
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if(countNumInfected() < numRows * numCols) {
                ...
                while(infected < 2) {
                    ...
                    if(!people[randX][randY].isInfected()) {
                        infect(randX, randY);
                        infected++;
                    }
                }
    ...
    private class StepListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            spread();
            updateRates();    
            // update the rate of all Persons in people[][], 
            // including increase of immune and infect rate if infected; and the gradual decrease of immune rate if healthy
            numCycles++;
    ...
    //Left-click to infect
    public void mouseClicked(MouseEvent e) {
        //if currRow and currCol are valid indices
    ...
    //Hover over grid block to check status
        ...
        for(int r=0; r<grid.length; r++) {
            for(int c=0; c<grid[0].length; c++) {
                if (e.getSource() == grid[r][c]) {
                    ...
                    //update the right-hand status display to show the status of the Person at grid[currRow][currCol]
                    updateStatusDisplay(r, c, people[r][c], false);
                }
            }
        }
    }

    // Resets status display to blank if mouse exits grid; 
    // this is to prevent unnecessary errors when the mouse is not hovering over any part of the grid
