...
{	
   ...
   private static boolean allowFall;               //do we allow particles to be pulled down by gravity or frozen in place?
 ...
 //post:  removes all plaforms (White colored particles)
  ...
  //post:  display all snow, platforms and instructions on the screen
   public static void showBoard(Graphics g)	
   {
      ...
      g.drawString("P  :   PAUSED:"+(!allowFall), x, y+=textSize);
      ...
      g.drawString("*******MOUSE*******", x, y+=textSize);
...
//post:  performs action depending on what key the user hit
   public void processUserInput(int k, boolean shiftPressed)
   {
      ...
      else if(k==KeyEvent.VK_P)								//freeze snow or allow it to fall
      {
         allowFall = !allowFall;
      }
 ...
 //***BEGIN MOUSE STUFF***
   private class Listener implements ActionListener
   {
      public void actionPerformed(ActionEvent e)	//this is called for each timer iteration
      {
         ...
         if(changeIndex > 0 && rain && allowFall)
         {
            ...
            if(changeIndex == 2 && Math.random() < 0.002)   
            {                                            //random color change is less frequent
               currentColor = randomColor();
               red = currentColor.getRed();
               green = currentColor.getGreen();
               blue = currentColor.getBlue();
         ...
         if(freqIndex > 0 && allowFall)                  //forcast might change
         {
         ...
         if(!winds[windIndex].contains("STEADY") && allowFall)
         {
            if(winds[windIndex].contains("VARIABLE"))
            {                                            //allow possibility for wind to change
   ...
   public void mouseDragged( MouseEvent e)
   {
      ...
      else if(cursorMode==DRAW && isValid(mouseY, mouseX))     
      {                                //paint tool to make platforms
