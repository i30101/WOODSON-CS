...
{      
  //to test reading in player information from a file.
   public static void main(String [] arg)
   {
      ...
      Player attempt = FileManager.readPlayerFromFile("data/testPlayerFile1.txt");
      if(attempt == null)
      {
      ...
      else
      {
         ...
         String fromFile = readFile("data/testPlayerFile1.txt");
      ...
      if(pass)
      {
         attempt = FileManager.readPlayerFromFile("data/testPlayerFile2.txt");
         if(attempt == null)
         {
         ...
         {
            String fromYourMethod = attempt.toString();
            String fromFile = readFile("data/testPlayerFile2.txt");
            if(fromYourMethod.equals(fromFile))
            {
   ...
   public static Player readPlayerFromFile(String fileName) {
      try {
         ...
         if(input.hasNextLine()) {  
            ...
            //******************************************************************/
            mapMark = new int[3];
            String[] mapMarkStrings = line.split(" ", -1);
            for(int i = 0; i < 3; i++) {
               mapMark[i] = Integer.parseInt(mapMarkStrings[i]);
            }
         ...
         //code to read potions from file: Line 4
            ...
            String[] potionsStrings = line.split(" ", -1);
            potions = new int[potionsStrings.length];
            for(int i = 0; i < potionsStrings.length; i++) {
               potions[i] = Integer.parseInt(potionsStrings[i]);
            }
            ...
            String[] flowersStrings = line.split(" ", -1);
            flowerBoxCount = new int[flowersStrings.length];
            for(int i = 0; i < flowersStrings.length; i++) {
               flowerBoxCount[i] = Integer.parseInt(flowersStrings[i]);
            }
            //******************************************************************/
         ...
         //code to read specificExperience from file: Line 6
            ...
            String[] experienceStrings = line.split(" ", -1);
            specificExperience = new int[experienceStrings.length];
            for(int i = 0; i < experienceStrings.length; i++) {
               specificExperience[i] = Integer.parseInt(experienceStrings[i]);
            }
            //******************************************************************/
            ...
            String[] statsStrings = line.split(" ", -1);
            stats = new int[statsStrings.length];
            for(int i = 0; i < statsStrings.length; i++) {
               stats[i] = Integer.parseInt(statsStrings[i]);
            }
            //******************************************************************/
            ...
            String[] infoIndexStrings = line.split(" ", -1);
            infoIndexes = new int[infoIndexStrings.length];
            for(int i = 0; i < infoIndexStrings.length; i++) {
               infoIndexes[i] = Integer.parseInt(infoIndexStrings[i]);
            }
            //******************************************************************/
          ...
          //code to read spellHotKeys from file: Line 9
            ...
            String[] spellKeyStrings = line.split(" ", -1);
            spellHotKeys = new int[spellKeyStrings.length];
            for(int i = 0; i < spellKeyStrings.length; i++) {
               spellHotKeys[i] = Integer.parseInt(spellKeyStrings[i]);
            }
            //******************************************************************/
            ...
            String[] potionKeyStrings = line.split(" ", -1);
            potionHotKeys = new int[potionKeyStrings.length];
            for(int i = 0; i < potionKeyStrings.length; i++) {
               potionHotKeys[i] = Integer.parseInt(potionKeyStrings[i]);
            }
            //******************************************************************/
          ...
          //code to read weaponHotKeys from file: Line 11
            ...
            String[] weaponKeyStrings = line.split(" ", -1);
            int weaponKeysNum = weaponKeyStrings.length / 2;
            weaponHotKeys = new Point[weaponKeysNum];
            for(int i = 0; i < weaponKeysNum; i++) {
               weaponHotKeys[i] = new Point(Integer.parseInt(weaponKeyStrings[i * 2]), Integer.parseInt(weaponKeyStrings[i * 2 + 1]));
            }
            //******************************************************************/
          ...
          //code to read effects from file: Line 12
            ...
            String[] effectsStrings = line.split(", ", -1);
            effects = new ArrayList<String>();
            for(String effect : effectsStrings) {
               effects.add(effect);
            }
            //******************************************************************/
            if(effects==null)
            ...
            //                                 as an example, let's assume line stores:                           "[images/characters/player/saber1.Gif, images/characters/player/saber2.Gif, images/characters/player/saber3.Gif, images/characters/player/saber4.Gif]"
            //initialize the fileNames ArrayList               
            ...
            //separate elements around ','                     (split) String [] parts stores 4 elements:         parts[0]->"images/characters/player/saber1.Gif", parts[1]->" images/characters/player/saber2.Gif", parts[2]->" images/characters/player/saber3.Gif" and parts[3]->" images/characters/player/saber4.Gif"
            ...
            line = line.replace("[", "").replace("]", "");
            String[] fileNameStrings = line.split(", ", -1);
          ...
          //code to read items from file: Line 14
            ...
            line = line.replace("[", "").replace("]", "");
            String[] itemStrings = line.split(", ", -1);
            ...
            line = line.replace("[", "").replace("]", "");
            String[] spellStrings = line.split(", ", -1);
          ...
          //code to read teleporter memory from file: Line 16
            ...
            line = line.replace("[", "").replace("]", "");
            String[] teleporterStrings = line.split(", ", -1);
            if(line.length() > 0) {
               for(String teleporter : teleporterStrings) {
                  String[] tpData = teleporter.replace("(", "").replace(")", "").split(" ", -1);
                  memory.add(new Teleporter(Integer.parseInt(tpData[0]), Integer.parseInt(tpData[1]), Integer.parseInt(tpData[2]), tpData[3]));
               }
            }
            //******************************************************************/
            if(memory==null)
          ...
          //code to read armorSet and armorFreq from file: Line 17
            ...
            String[] armorStrings = line.replace("[", "").replace("]", "").split(", ", -1);
            armorSet = new ArrayList<Armor>();
            armorFreq = new ArrayList<Integer>();
            for(String armor : armorStrings) {
               String[] armorData = armor.split(":", -1);
               armorSet.add(new Armor(armorData[0]));
               armorFreq.add(Integer.parseInt(armorData[1]));
            }
            //******************************************************************/
          ...
          //code to read weapons and weaponFrequencies from file: Lines 18-32
            ...
            for(int i = 0; i < 15; i++) {
               ...
               if(pos >= 0)
                  line = line.substring(0,pos).trim();
               line = line.replace("[", "").replace("]", "");
               ArrayList<Weapon> lineWeapons = new ArrayList<Weapon>();
               ArrayList<Integer> lineFrequencies = new ArrayList<Integer>();
               if(line.length() > 0) {
                  String[] weaponStrings = line.split(", ", -1);
                  for(String weaponString : weaponStrings) {
                     String[] weaponStringList = weaponString.split(":", -1);
                     lineWeapons.add(new Weapon(weaponStringList[0]));
                     lineFrequencies.add(Integer.parseInt(weaponStringList[1]));
                  }
               }
               weapons[i] = lineWeapons;
               weaponFrequencies[i] = lineFrequencies;
            }
            //**************************DONE!************************************/ 
