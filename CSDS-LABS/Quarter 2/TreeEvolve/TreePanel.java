//Douglas R Oberle - doberle@fcps.edu.
import javax.swing.*;
import java.awt.*;
...
Tree mutate(int mutationRate)		given the current tree, return an almost-clone of the tree where each field can vary by mutationRate.
int compareTo(Object other)		Trees compare to one another by how fit they are (one that is more fit is > one that is less fit).

**************************************************/

public class TreePanel extends JPanel {
    private static Tree [] forest;             //sample set of Trees (most fit is at forest[0]).
    ...
    private static void advanceGeneration() {
        ...
        Tree fittest = forest[0];

        // obtain fittest tree+
        for (Tree tree : forest) {
            if (tree.compareTo(fittest) > 0) {
                fittest = tree;
            }
        }

        // create new forest
        Tree[] newForest = new Tree[SAMPLE_SIZE];

        // add fittest tree to front
        newForest[0] = fittest;

        // fill rest of forest with mutated children
        for (int i = 1; i < newForest.length; i++) {
            newForest[i] = fittest.mutate(MUTATION_RATE);
        }
    ...
    */
    public void paintComponent(Graphics g) {
        ...
        g.drawString("SampleSize (" + SAMPLE_SIZE + 
            "):1+2     MutationRate(" + MUTATION_RATE + 
            "):3+4     Leaf RGB(" + red + "," + green + "," + blue + 
            "):5+6,7+8,9+0                                                (Width):" + forest[0].getPixelWidth() + 
            "        (Height):"+forest[0].getPixelHeight() + 
            "     (Number Generations):" + numGenerations + 
            "     (How fit):" + clipNum(forest[0].howFit()), 0, 20
        );   
        g.drawString("TrunkLength("+clipNum(forest[0].getLength()) + 
            "):Q+W     LengthChange("+clipNum(forest[0].getLengthChange()) + 
            "):E+R      TrunkWidth("+clipNum(forest[0].getWidth()) + 
            "):A+S     WidthChange("+clipNum(forest[0].getWidthChange()) + 
            "):D+F     Degrees("+clipNum(forest[0].getDegreesChange()) + 
            "): Z+X     NumBranches("+forest[0].getNumBranches() + 
            "): C+V     Smaller Increments:SHIFT     Move:Arrow Keys     BackColor:B+N", 0, 40);  
        g.setColor(previous);
    }
    ...
    public class Keyboard implements KeyListener {
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if(e.getKeyCode()==KeyEvent.VK_2) {     //Sample Size (2-25)
                ...
                newForest[SAMPLE_SIZE - 1] = newForest[0].mutate(MUTATION_RATE);
                forest = newForest;
                //******************************************************************/
                repaint();
                return;
